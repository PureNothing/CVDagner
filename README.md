...
| | |
|-|-|
| <img src="assets/front_white.jpg" width="260"/> | <img src="assets/front_black.jpg" width="260"/> |

### Интерфейкс Телеграм Бота:
...
```mermaid
flowchart TD
    OP([👤 Оператор]) -->|загружает видео| FE[🖥️ frontend_VideoProduce\nStreamlit UI :8501]
    FE -->|POST /video_upload/camera_id| BV[🎬 backend_VideoProduce\nFastAPI :8000]

    BV -->|сохраняет оригинал| S3_V[(🪣 MinIO\nбакет: videos)]
    BV -->|записывает видео| PG_V[(🐘 PostgreSQL\nVideos)]
    BV -->|нарезает 1 кадр/сек PyAV| BV
    BV -->|сохраняет фреймы| S3_F[(🪣 MinIO\nбакет: frames)]
    BV -->|записывает фреймы| PG_F[(🐘 PostgreSQL\nFrames\nstatus: not_processed)]
    BV -->|публикует путь к фрейму| K1[[📨 Kafka\nready_to_detect]]

    K1 -->|читает| DG[🔍 DetectGraphApi\nFastAPI :8001\nmax_workers=3]
    DG -->|скачивает фрейм| S3_F
    DG -->|POST /detect base64| BS[🤖 BentoService\nYOLOE ONNX :7777]
    BS -->|боксы + классы + scores| DG

    DG -->|рисует боксы PIL| DG
    DG -->|сохраняет аннотированный фрейм| S3_D[(🪣 MinIO\nбакет: detected)]
    DG -->|записывает результат| PG_D[(🐘 PostgreSQL\nDetected_frames\nDetected_objects)]
    DG -->|публикует результаты детекции| K2[[📨 Kafka\nalerts_topic]]
    DG -->|публикует путь фрейма| K3[[📨 Kafka\nframe_detected]]

    K3 -->|читает| BV
    BV -->|обновляет статус| PG_F2[(🐘 PostgreSQL\nFrames\nstatus: processed)]

    K2 -->|читает| AS[🚨 AlertsService\nFastAPI :8002]
    AS -->|читает пороги камеры| RJ[(📋 rules.json\nпороги + координаты\n+ местность)]
    AS -->|если count >= threshold| K4[[📨 Kafka\nreal_alerts]]

    K4 -->|читает| AI[🧠 AIService\nFastAPI :8003\nGroq API]
    K4 -->|читает| TG[📱 telegram_bot\nAiogram]

    AI -->|Tool Calling агент| AI
    AI -->|запрашивает координаты| AS
    AI -->|публикует решение| K5[[📨 Kafka\nagent_actions]]
    K5 -->|читает| TG

    TG -->|мгновенный алерт| OP
    TG -->|решение агента| OP

    OP -->|/settings + ключ| TG
    TG -->|POST /ai_rules_message| AI2[🧠 AIService\nStructured Output агент]
    AI2 -->|POST /change_rules| AS
    AS -->|обновляет пороги| RJ

    OP -->|запрос отчёта по камере| TG
    TG -->|GraphQL getCameraReport| DG
    TG -->|GET /get_coordinates\nGET /get_restrictions| AS
    TG -->|форматированный отчёт| OP

    CE[⏰ Celery Beat\nкаждые 10 минут] -->|запускает задачу| CT[Celery Worker]
    CT -->|GraphQL getGeneralReport| DG
    CT -->|GET координаты + местность| AS
    CT -->|POST /ai_report_parse| AI3[🧠 AIService\nReport агент]
    AI3 -->|аналитический доклад| CT
    CT -->|доклад| OP
```
